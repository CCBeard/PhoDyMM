...
if npl > 4:
  ...
  for i in range(npl):
    allparam.hist('M$_%s$' % alphabet[i], ax=axes[int(i/2), i % 2], density=1)
  axes[sum(divmod(npl,2))-1,0].set_xlabel('M$_{Earth}$', fontsize=14)
  axes[int(npl/2-1),1].set_xlabel('M$_{Earth}$', fontsize=14)
...
if npl > 4:
  ...
  for i in range(npl):
    allparam.hist('R$_%s$' % alphabet[i], ax=axes2[int(i/2), i % 2], density=1)
  axes2[sum(divmod(npl,2))-1,0].set_xlabel('R$_{Earth}$', fontsize=14)
  axes2[int(npl/2-1),1].set_xlabel('R$_{Earth}$', fontsize=14)
  plt.tight_layout()
...
if npl>4:
  ...
  for i in range(npl):
    allparam.hist(r'$\rho$$_%s$' % alphabet[i], ax=axes3[int(i/2), i % 2], density=1)
  axes3[sum(divmod(npl,2))-1,0].set_xlabel('g cm$^{-3}$', fontsize=14)
  axes3[int(npl/2-1),1].set_xlabel('g cm$^{-3}$', fontsize=14)
  plt.tight_layout()
